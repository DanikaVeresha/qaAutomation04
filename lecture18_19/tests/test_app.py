...
def test_data_user_username(username, password, email):
...
@pytest.mark.parametrize('password', ['jon1234'])
@pytest.mark.parametrize('email', ['jonesmith@gmail.com'])
def test_data_user_password(username, password, email):
    user = User(username, password, email)
...
@pytest.mark.parametrize('password', ['jon1234'])
@pytest.mark.parametrize('email', ['jonesmith@gmail.com'])
def test_data_user_email(username, password, email):
    user = User(username, password, email)
...
def test_add_user(username, password, email):
    ...
    assert user.get_user_data() == f'User: {user.users}', 'User is not added'
...
def test_registration(username, password, email):
    user = User(username, password, email)
    user.registration()
    assert username in user.users[0].values(), 'User is not added'
...
def test_create_order(username, password, email):
    ...
    assert username in user.orders[0].values(), 'Order is not added'
    assert username in user.users[0].values(), 'User is not added'
...
def test_delete_order(username, password, email):
    ...
    assert username not in user.users, 'User is not deleted'
