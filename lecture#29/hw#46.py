...
def client_code(directory, number_of_files, size, letter_to_find, number_of_threads):
    ...
    print(f'-> The results are equal: {equal_res}')
    return f'{number_of_files} -> {size} | {counter} -> {time3} | {total_count_letters} -> {time4} |'


if __name__ == '__main__':
    print(f'-> Number of files/size created: '
          f'{file_generator("files", 12, 100)}\n')
    ...
    print(f'-> Result Client Code: \n'
          f'\tNumber of files -> Size | Number of letters -> Time "Thread" | Number of letters -> Time "Threads" | \n'
